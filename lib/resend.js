...
/**
 * Send workspace invite email (Step 4 — Owner invites Member or User)
 * @param {string} to - invitee email
 * @param {string} workspaceName
 * @param {string} role - 'member' | 'user'
 * @param {string} inviterEmail
 */
export async function sendInviteEmail(to, workspaceName, role, inviterEmail) {
  await resend.emails.send({
    from: FROM,
    to,
    subject: `${inviterEmail} invited you to ${workspaceName} on Loopback`,
    html: `
<div style="font-family:-apple-system,sans-serif;max-width:480px;margin:40px auto;padding:32px;border:1px solid #e5e7eb;border-radius:12px">
  <h2 style="font-size:18px;font-weight:700;color:#0d0d0d;margin:0 0 12px">You've been invited to ${workspaceName}</h2>
  <p style="font-size:14px;color:#6b7280;line-height:1.6;margin:0 0 16px">
    ${inviterEmail} added you as a <strong>${role}</strong> on their Loopback workspace. Create your account with this email address to get access.
  </p>
  <a href="https://www.fuffad.com/signup" style="display:inline-block;background:#3b7eff;color:#fff;text-decoration:none;font-weight:600;font-size:14px;padding:10px 20px;border-radius:8px">Create your account</a>
